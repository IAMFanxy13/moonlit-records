...
The score atelier gains a prominent `Paste Moonlit Score Code` area above the image/PDF fallback. Pasting does not execute anything. `Prepare this code` validates and compiles the text locally, persists the song in the existing private library, and shows the same ready-to-perform card as image recognition.
